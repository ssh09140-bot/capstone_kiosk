 ...
 */
export async function generateHygieneCheck(recentOrders: any[]): Promise<string | null> {
  const ordersSummary = recentOrders.map(o =>
    `- ${o.productName} (${o.quantity}잔) [옵션: ${o.options || '없음'}]`
  ).join('\n');
