...
const generateRecommendations = async (storeId) => {
    ...
    // 1. 핵심 재고 품목 정보 조회
    const inventoryItem = await db_1.default.inventory.findFirst({
    ...
    const productUsages = await db_1.default.productInventoryUsage.findMany({
    ...
    const pastOrders = await db_1.default.orderItem.findMany({
        where: {
            order: {
                storeId,
                createdAt: { gte: (0, date_fns_1.subDays)(new Date(), ANALYSIS_DAYS) },
            },
    ...
    const bestSupplierInfo = await db_1.default.supplierInventory.findFirst({
        where: { inventoryId: inventoryItem.id },
