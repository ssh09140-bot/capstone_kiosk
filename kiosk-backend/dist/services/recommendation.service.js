...
async function getWeatherForecast(city, days) {
    ...
    try {
        ...
        for (let i = 1; i <= days; i++) {
            ...
            // 정오(12:00) 기준 예보 사용
            const dayForecast = response.data.list.find(item => item.dt_txt.startsWith(targetDateString) && item.dt_txt.endsWith('12:00:00'));
            if (dayForecast) {
 ...
 * 도시 이름 매핑
 */
async function getCityForWeather(storeId) {
 ...
 */
function calculateDayOfWeekFactors(dailyUsages, startDate) {
    const daySums = [0, 0, 0, 0, 0, 0, 0];
    const dayCounts = [0, 0, 0, 0, 0, 0, 0];
    dailyUsages.forEach((usage, index) => {
        const date = (0, date_fns_1.addDays)(startDate, index);
        const day = (0, date_fns_1.getDay)(date); // 0 (Sun) ~ 6 (Sat)
        daySums[day] += usage;
        dayCounts[day]++;
    });
...
const generateRecommendations = async (storeId) => {
    ...
    for (const inv of inventories) {
        ...
        orderItems.forEach(oi => {
            ...
            if (usageInfo) {
                const usagePerItem = (0, unitConversionService_1.convertToBaseUnit)(usageInfo.usageAmount, usageInfo.usageUnit, inv.unit);
    ...
    analysisData.forEach(item => {
        ...
        if (ratio <= 0.7)
            abcMap.set(item.id, 'A'); // 상위 70% 가치 -> A
        else if (ratio <= 0.9)
            abcMap.set(item.id, 'B'); // 다음 20% -> B
        else
