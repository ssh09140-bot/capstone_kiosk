...
function getLocalExternalIp() {
    ...
    const filtered = candidates.filter(ip => !ip.startsWith('192.168.56.') &&
        !ip.startsWith('172.'));
...
router.post('/payment/toss/prepare', async (req, res) => {
    ...
    const { amount, orderName, storeId, items } = req.body;
        ...
        for (const item of items) {
            ...
            if (product && product.inventoryUsages) {
                for (const usage of product.inventoryUsages) {
                    // 사용량을 재고 기본 단위로 변환
                    const usageAmount = (0, unitConversionService_1.convertToBaseUnit)(usage.usageAmount, usage.usageUnit, usage.inventory.unit);
                    // 최저가 공급업체 찾기
...
router.post('/payment/toss/confirm', async (req, res) => {
    ...
    try {
        const response = await (0, node_fetch_1.default)('https://api.tosspayments.com/v1/payments/confirm', {
            method: 'POST',
            headers: {
                'Authorization': `Basic ${Buffer.from(`${TOSS_SECRET_KEY}:`).toString('base64')}`,
                'Content-Type': 'application/json',
